...
analysis_IDs <- rownames(analysiis_label_df) 
...
# Basic violin plot for critical features
critical_features_df <- read.csv(paste0(f_dir3, "/Exp1_important_fs.csv"),stringsAsFactors = F)
critical_features <- critical_features_df$x
...
for (f in 1:length(critical_features)){
  ...
  voilin_plot_data$New_Trend_Label[grp0_idex] <- "(Grp 0) start non-high, \n stay non-high"
  voilin_plot_data$New_Trend_Label[grp1_idex] <- "(Grp 1) start non-high, \n get higher"
